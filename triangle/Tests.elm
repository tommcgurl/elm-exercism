...
tests =
    describe "triangleKind"
        ...
        , test "degenerate triangles occure when the sum of any two sides is equal to the length of the third" <|
            \() -> Expect.equal (Ok Degenerate) (triangleKind 4 2 2)
        , test "degenerate triangles occure when the sum of any two sides is equal to the length of the third" <|
            \() -> Expect.equal (Ok Degenerate) (triangleKind 4 2 2)
        , test "a  triangle is a degenerate if the sum of the first two sides equals the length of the third" <|
            \() -> Expect.equal (Ok Degenerate) (triangleKind 2 2 4)
        , test "a  triangle is a degenerate if the sum of the fist and third sides equals the length of the second" <|
            \() -> Expect.equal (Ok Degenerate) (triangleKind 2 4 2)
        ]
